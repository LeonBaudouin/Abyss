...
export default class ThreeScene {
  ...
  setupScene() {
    ...
    const fogColor = new THREE.Color(0x001824);
    
    this.scene.background = fogColor;
    this.scene.fog = new THREE.FogExp2(fogColor, 0.2);
    this.objects.forEach(obj => this.scene.add(obj.object3d));  
  }

  setupControls() {
    this.controls = new OrbitControls(this.cameraComponent.object3d, this.renderer.domElement);
  ...
  resizeCanvas() {
    this.renderer.setSize(window.innerWidth, window.innerHeight);
    this.cameraComponent.object3d.aspect = window.innerWidth / window.innerHeight;
